...
Note: If the user returns a correct copy of the given graph, then the system will print 1; in the case when an incorrect copy is generated or
when the user returns the original node, the system will print 0.
...
public:
    ...
    Node* cloneGraph(Node* node){
        ...
        for(Node* x : node->neighbors){
            ...
            if(m.count(x->val)!=0)
                head->neighbors.push_back(m[x->val]);
        }
